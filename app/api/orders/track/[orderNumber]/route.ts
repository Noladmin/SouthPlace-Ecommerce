...
export async function GET(
  ...
  try {
    ...
    const orderDetails = {
      ...
      waitingForPickupAt: order.waitingForPickupAt,
      pickedUpAt: order.pickedUpAt,
      outForDeliveryAt: order.outForDeliveryAt,
      deliveredAt: order.deliveredAt,
      deliveryFailedAt: order.deliveryFailedAt,
      cancelledAt: order.cancelledAt,
      activeDeliveryAssignment: order.activeDeliveryAssignmentId ? await prisma.deliveryAssignment.findUnique({
        where: { id: order.activeDeliveryAssignmentId },
        select: {
          assignmentType: true,
          riderName: true,
          providerName: true,
